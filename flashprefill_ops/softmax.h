...
struct Softmax {
    ...
    // Finalize with learnable sink (GPT-OSS style per-head sink logit).
    // sink_val is the raw (pre-softmax-scale) learnable logit for this head.
    // It contributes to the softmax denominator (row_sum) but not to the output (acc_o).
    // When sink_val is -INFINITY (e.g. split_idx != 0), the contribution is zero.
    __forceinline__ __device__ TensorT finalize(float const final_scale, float const sink_val) {
        SumOp<float> sum_op;
        quad_allreduce_(row_sum, row_sum, sum_op);
        TensorT scores_scale;
        #pragma unroll
