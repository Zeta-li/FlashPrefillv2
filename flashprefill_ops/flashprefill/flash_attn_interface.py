...
if not USE_TRITON_ROCM and getattr(torch.version, 'hip', None) is not None:
    try:
        import flashprefill._C
    except ImportError:
        warnings.warn("flashprefill._C (which has ROCm/HIP kernels) not found, falling back to Triton implementation")
...
if USE_TRITON_ROCM:
    ...
    import flashprefill._C # Registers operators with PyTorch
...
@torch.library.custom_op("flashprefill::_flash_attn_forward", mutates_args=(), device_types="cuda")
...
@torch.library.register_fake("flashprefill::_flash_attn_forward")
...
@torch.library.custom_op("flashprefill::_flash_attn_backward", mutates_args=("dq", "dk", "dv"), device_types="cuda")
def _flash_attn_backward(
    ...
    sm_margin: int = 0,
    bwd_block_sparse_cu: Optional[torch.Tensor] = None,
    bwd_block_sparse_idx: Optional[torch.Tensor] = None,
    bwd_max_k_tiles: Optional[int] = None,
) -> torch.Tensor:
    ...
    softmax_d, *rest = flash_attn_3_gpu.bwd(
        ...
        sm_margin,
        bwd_block_sparse_cu,
        bwd_block_sparse_idx,
        bwd_max_k_tiles,
    )
    return softmax_d


@torch.library.register_fake("flashprefill::_flash_attn_backward")
...
class FlashAttnVarlenFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if softmax_scale is None:
            softmax_scale = (q.shape[-1] + (qv.shape[-1] if qv is not None else 0)) ** (-0.5)
        has_block_sparse = block_sparse_cu is not None
        ...
        out, softmax_lse, *rest = _flash_attn_forward(
            ...
            softcap=softcap,
            num_splits=num_splits,
            pack_gqa=True if has_block_sparse else None,
        ...
        if has_block_sparse:
            ctx.save_for_backward(q, k, v, out, softmax_lse, cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k,
                                  block_sparse_cu, block_sparse_idx, cu_q_tiles)
        else:
            ctx.save_for_backward(q, k, v, out, softmax_lse, cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k)
        ...
        ctx.deterministic = deterministic
        ctx.sm_margin = sm_margin
        ctx.has_block_sparse = has_block_sparse
        ctx.total_q_tiles = int(total_q_tiles) if has_block_sparse and total_q_tiles is not None else None
        return (out, softmax_lse) if return_softmax else out

    @staticmethod
    def backward(ctx, dout, *args):
        if getattr(ctx, "has_block_sparse", False):
            (q, k, v, out, softmax_lse, cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k,
             block_sparse_cu, block_sparse_idx, cu_q_tiles) = ctx.saved_tensors
        else:
            q, k, v, out, softmax_lse, cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k = ctx.saved_tensors
        assert ctx.attention_chunk == 0, "FA3 backward does not support attention_chunk"
        if getattr(ctx, "has_block_sparse", False):
            if not ctx.causal:
                raise NotImplementedError("block sparse varlen backward supports causal=True only")
            if ctx.window_size != (-1, -1):
                raise NotImplementedError("block sparse varlen backward does not support sliding window")
            if ctx.softcap != 0.0 or ctx.deterministic or ctx.sm_margin != 0:
                raise NotImplementedError("block sparse varlen backward supports only softcap=0, deterministic=False, sm_margin=0")
            if q.dtype not in (torch.float16, torch.bfloat16):
                raise NotImplementedError("block sparse varlen backward supports fp16/bf16 only")
            if q.shape[-1] != v.shape[-1] or q.shape[-1] not in (128, 256):
                raise NotImplementedError("block sparse varlen backward currently supports hdim128/256 with equal qk/v dim")
            try:
                from .flash_block_sparse_bwd_index import build_block_sparse_bwd_index
            except ImportError:
                from flash_block_sparse_bwd_index import build_block_sparse_bwd_index
            bwd_block_sparse_cu, bwd_block_sparse_idx, bwd_max_k_tiles = build_block_sparse_bwd_index(
                block_sparse_cu, block_sparse_idx, cu_q_tiles, cu_seqlens_q, cu_seqlens_k,
                q.shape[1], k.shape[1], ctx.total_q_tiles, ctx.max_seqlen_k,
            )
        else:
            bwd_block_sparse_cu = bwd_block_sparse_idx = bwd_max_k_tiles = None
        dq, dk, dv = torch.empty_like(q), torch.empty_like(k), torch.empty_like(v)
        _flash_attn_backward(
            dout,
            q,
            ...
            ctx.deterministic,
            ctx.sm_margin,
            bwd_block_sparse_cu=bwd_block_sparse_cu,
            bwd_block_sparse_idx=bwd_block_sparse_idx,
            bwd_max_k_tiles=bwd_max_k_tiles,
        )
        dq = dq[..., : q.shape[-1]]  # We could have padded the head dimension
        dk = dk[..., : k.shape[-1]]
        dv = dv[..., : v.shape[-1]]
        return dq, dk, dv, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None
...
def flash_attn_varlen_func(
    ...
    if block_sparse_cu is not None:
        if block_sparse_idx is None or total_q_tiles is None or cu_q_tiles is None:
            raise ValueError("block_sparse_cu, block_sparse_idx, total_q_tiles and cu_q_tiles must be provided together")
        block_sparse_cu, block_sparse_idx, cu_q_tiles = [
            maybe_contiguous(x) for x in (block_sparse_cu, block_sparse_idx, cu_q_tiles)
        ]
        assert block_sparse_cu is not None and block_sparse_idx is not None and cu_q_tiles is not None
        batch_size = cu_seqlens_q.shape[0] - 1
        num_kv_heads = k.shape[-2]
        total_q_tiles = int(total_q_tiles)
        for name, tensor in (("block_sparse_cu", block_sparse_cu), ("block_sparse_idx", block_sparse_idx), ("cu_q_tiles", cu_q_tiles)):
            if not tensor.is_cuda:
                raise ValueError(f"{name} must be a CUDA tensor")
            if tensor.dtype != torch.int32:
                raise ValueError(f"{name} must have dtype torch.int32")
        if cu_q_tiles.dim() != 1 or cu_q_tiles.shape[0] != batch_size + 1:
            raise ValueError("cu_q_tiles must have shape (batch_size + 1,)")
        expected_cu = num_kv_heads * total_q_tiles + 1
        if block_sparse_cu.dim() != 1 or block_sparse_cu.shape[0] != expected_cu:
            raise ValueError(f"block_sparse_cu must have shape ({expected_cu},) for PackGQA CSR")
        if block_sparse_idx.dim() != 1:
            raise ValueError("block_sparse_idx must be a 1D tensor")
    elif any(x is not None for x in (block_sparse_idx, total_q_tiles, cu_q_tiles)):
        raise ValueError("block_sparse_cu, block_sparse_idx, total_q_tiles and cu_q_tiles must be provided together")
...
def flash_attn_with_kvcache(
    ...
    k_mean=None,     # (batch, max_k_blocks, nheads_k, headdim) pooled block means for the unselected-block correction
    v_mean=None,     # (batch, max_k_blocks, nheads_k, headdim_v)
    mean_k_block_size=0,  # logical block tokens (64 * n_sub); 0 disables the correction
...
def build_block_sparse_index(
    ...
    """Build block sparse index in-place.

    Computes a score-based block sparse index using PackGQA and writes the CSR
    compact index into block_sparse_cu and block_sparse_idx tensors.

    Args:
        q: (total_q, num_q_heads, d) query tensor for varlen, or (b, s_q, h, d) for non-varlen.
        k_pool: (num_pages, page_size, num_k_heads, d) paged KV cache pool (keys).
        page_table: (batch, max_num_pages_per_seq) int32 page table.
        cu_seqlens_q: (batch+1,) int32 cumulative query sequence lengths, or None for non-varlen.
        kv_seqlens: (batch,) int32 actual KV cache lengths per batch.
        cu_q_tiles: (batch+1,) int32 cumulative Q tile count per batch.
        block_sparse_cu: (num_k_heads * total_q_tiles + 1,) int32, CSR prefix sum (output).
        block_sparse_idx: (pre-allocated,) int32, flat compact index (output).
        batch_size, num_q_heads, num_k_heads, total_q_tiles: metadata.
        max_seqlen_q, max_seqlen_k: max sequence lengths.
        alpha: threshold ratio for top-alpha selection.
        attention_sink: number of leading K blocks always selected.
        window_size: sliding window size in K-block units.
        last_n_blocks: number of trailing Q tiles always selecting all K blocks.
        is_causal: whether to apply causal mask.
        scale: softmax scale. If None, uses 1/sqrt(d).
        k_scale: FP8 descale factor for K (1.0 for bf16/fp16).
        is_fp8_kv: whether KV is in FP8 e4m3 format.
    """
    if scale is None:
        head_size = q.shape[-1]
        scale = 1.0 / (head_size ** 0.5)

    raise NotImplementedError("build_block_sparse_index is disabled (kernel has bug, not compiled)")
    # flash_attn_3_gpu.build_block_sparse_index(
    #     q,
    #     k_pool,
    #     page_table,
    #     cu_seqlens_q,
