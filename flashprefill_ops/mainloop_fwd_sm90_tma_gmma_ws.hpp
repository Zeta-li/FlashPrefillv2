...
template <int Stages, class ClusterShape_, class TileShape_MNK_, int kHeadDimV, class Element_, class ElementAccum_, class ArchTag_,
        ...
        bool MmaPV_is_RS, bool IntraWGOverlap, bool PackGQA_, bool Split_, bool V_colmajor_, bool Has_BlockSparse_, bool Has_Sink_>
struct CollectiveMainloopFwdSm90 {
    ...
    static constexpr bool Has_BlockSparse = Has_BlockSparse_;
    ...
    struct Arguments {
        ...
        // Block sparse attention (CSR compact index, organized by KV head)
        int const* const ptr_block_sparse_cu = nullptr;     // [num_kv_heads * total_q_tiles + 1] — CSR prefix-sum
        int const* const ptr_block_sparse_idx = nullptr;    // [block_sparse_cu[-1]] — flat compact index, ascending
        int const total_q_tiles = 0;                        // total Q tiles across all batches (head stride)
        int const* const cu_q_tiles = nullptr;              // [batch + 1] — cumulative Q tile count per batch (varlen offset)
        // Zero-order mean correction for unselected blocks, fused into the
        // fwd epilogue after the mainloop (Sol-Attn style). Pooled per-block
        // means: [batch, max_k_blocks, nheads_kv, head_dim(/_v)].
        Element const* const ptr_k_mean = nullptr;
        Element const* const ptr_v_mean = nullptr;
        int64_t const k_mean_batch_stride = 0, k_mean_block_stride = 0, k_mean_head_stride = 0;
        int64_t const v_mean_batch_stride = 0, v_mean_block_stride = 0, v_mean_head_stride = 0;
        int const mean_k_block_size = 0;                    // logical block tokens (= 64 * n_sub); 0 disables
        int const mean_n_sub = 1;                           // physical 64-token tiles per logical block
        // Learnable sink (GPT-OSS style per-head sink logit, [num_heads])
        Element const* const ptr_sinks = nullptr;
    };
    struct Params {
        Element const* const ptr_Q;
        ShapeQKV const shape_Q;
        ...
        int const* const leftpad_k = nullptr;
        int const *const seqlens_rotary = nullptr;
        // Block sparse attention (CSR compact index, organized by KV head) — device side
        int const* const ptr_block_sparse_cu = nullptr;
        int const* const ptr_block_sparse_idx = nullptr;
        int const total_q_tiles = 0;
        int const* const cu_q_tiles = nullptr;
        // Zero-order mean correction for unselected blocks — device side
        Element const* const ptr_k_mean = nullptr;
        Element const* const ptr_v_mean = nullptr;
        int64_t const k_mean_batch_stride = 0, k_mean_block_stride = 0, k_mean_head_stride = 0;
        int64_t const v_mean_batch_stride = 0, v_mean_block_stride = 0, v_mean_head_stride = 0;
        int const mean_k_block_size = 0;
        int const mean_n_sub = 1;
        // Learnable sink — device side
        Element const* const ptr_sinks = nullptr;
    };
    static Params
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_Q, args.shape_Q, args.stride_Q, shape_Q_packed, stride_Q_packed,
                ...
                args.ptr_block_sparse_cu, args.ptr_block_sparse_idx,
                args.total_q_tiles, args.cu_q_tiles,
                args.ptr_k_mean, args.ptr_v_mean,
                args.k_mean_batch_stride, args.k_mean_block_stride, args.k_mean_head_stride,
                args.v_mean_batch_stride, args.v_mean_block_stride, args.v_mean_head_stride,
                args.mean_k_block_size, args.mean_n_sub,
                args.ptr_sinks};
    }
    ...
    load(Params const& params,
        ...
        // For block sparse, the n_block range is from CSR index, not from get_n_block_min_max.
        if constexpr ((Is_causal || Is_local || Varlen || Split) && !Has_BlockSparse) {
            if (n_block_max <= n_block_min) {
                scheduler_prefetch();
                return;
            }
        }
        // Block sparse: lookup compact index list for this (batch, head_kv, m_block)
        // CSR format: g = total_q_tiles * bidh_kv + global_m_block
        // valid_start = cu[g], valid_end = cu[g+1], index_ptr[valid_start:valid_end] ascending
        int block_sparse_count = 0;
        int block_sparse_lo = 0;  // inclusive start in flat index array
        if constexpr (Has_BlockSparse) {
            // PackGQA=true:  segment organized by KV head, g = total_q_tiles * bidh + m_block
            // PackGQA=false: segment organized by Q head, g = total_q_tiles * bidh + m_block
            //   (matches Triton kernel format: each Q head has independent sparse index)
            int const bidh_kv_bs = bidh;
            int global_m_block = m_block;
            if constexpr (Varlen) {
                global_m_block += params.cu_q_tiles[bidb];
            }
            int g = params.total_q_tiles * bidh_kv_bs + global_m_block;  // segment id
            block_sparse_lo = params.ptr_block_sparse_cu[g];
            int block_sparse_hi = params.ptr_block_sparse_cu[g + 1];
            block_sparse_count = block_sparse_hi - block_sparse_lo;
            if (block_sparse_count == 0) {
                scheduler_prefetch();
                return;
            }
            // Split-aware partition: divide the sparse K-tile list among splits
            if constexpr (Split) {
                ...
                int num_splits_actual = num_splits_dynamic > 0 ? num_splits_dynamic : params.num_splits;
                if (num_splits_actual > 1 && block_sparse_count > 0) {
                    int blocks_per_split = cute::ceil_div(block_sparse_count, num_splits_actual);
                    int split_start = split_idx_actual * blocks_per_split;
                    int split_end = std::min(split_start + blocks_per_split, block_sparse_count);
                    if (split_start >= split_end) {
                        scheduler_prefetch();
                        return;
                    }
                    block_sparse_lo += split_start;
                    block_sparse_count = split_end - split_start;
                }
            }
            // First n_block to load (ascending order, so first = smallest n_block)
            // FA3 iterates from high to low n_block, so we reverse: start from last element
        }
        ...
        int n_block;
        if constexpr (Has_BlockSparse) {
            // sglang CSR: indices are ascending, FA3 iterates descending, so first = last element
            n_block = params.ptr_block_sparse_idx[block_sparse_lo + block_sparse_count - 1];
        } else {
        ...
        if constexpr (Has_BlockSparse) {
            // Software pipeline: prefetch next index before the loop starts
            int n_block_next = (block_sparse_count >= 2) ? params.ptr_block_sparse_idx[block_sparse_lo + block_sparse_count - 2] : 0;
            // Iterate compact index list (first element already loaded above)
            #pragma unroll (!Transpose_V && Use_TMA_KV ? 2 : 1)
            for (int i = block_sparse_count - 2; i >= 0; --i) {
                int n_block = n_block_next;
                // Prefetch next iteration's index (overlaps with TMA loads below)
                if (i > 0) { n_block_next = params.ptr_block_sparse_idx[block_sparse_lo + i - 1]; }
        ...
        // Sol-Attn style zero-order correction: stage the pooled K/V block
        // means through the same pipelines, as extra blocks right after the
        // selected ones (one pipeline slot per kBlockN-wide chunk of means).
        // The MMA side consumes the identical chunk sequence in
        // the consumer's correction fwd_step iterations -- both sides iterate
        // chunks in ascending
        // order and skip chunks where every block is selected, using the
        // same two-pointer scan over the ascending CSR, so the pipeline
        // state stays in lockstep.
        if constexpr (Has_BlockSparse && PagedKVNonTMA && !LargeHeadDimV && !Split && !HasQv && !AppendKV) {
            if (params.ptr_k_mean != nullptr && params.mean_k_block_size > 0) {
                static constexpr int kBlockN = get<1>(TileShape_MNK{});
                int const kB = params.mean_k_block_size;
                int const n_sub = params.mean_n_sub;
                int const seqlen_k = seqlen_info.seqlen_k;
                int const n_logical = (seqlen_k + kB - 1) / kB;
                int j_end = n_logical;
                if constexpr (Is_causal) {
                    int const prefix = seqlen_k - seqlen_info.seqlen_q;
                    int const q_pos_min = !PackGQA
                        ? m_block * get<0>(TileShape_MNK{})
                        : params.qhead_per_khead_divmod.divide(m_block * get<0>(TileShape_MNK{}));
                    // Only blocks fully visible to every row of this M tile
                    // are corrected; must match the consumer correction loop exactly.
                    j_end = std::min(j_end, (prefix + q_pos_min) / kB);
                }
                if (j_end > 0) {
                    Element const* k_mean_base = params.ptr_k_mean
                        + int64_t(bidb) * params.k_mean_batch_stride
                        + int64_t(bidh_kv) * params.k_mean_head_stride;
                    Element const* v_mean_base = params.ptr_v_mean
                        + int64_t(bidb) * params.v_mean_batch_stride
                        + int64_t(bidh_kv) * params.v_mean_head_stride;
                    // Explicit 16B cp.async loop: each chunk is addressed
                    // through the (swizzled) smem layout view, so bytes land
                    // exactly where the GMMA read views expect them. 16B
                    // chunks are never split by the smem swizzle.
                    auto cp_async_16B = [](void* smem_dst, void const* gmem_src, bool pred) {
                        uint32_t dst = cute::cast_smem_ptr_to_uint(smem_dst);
                        int const src_size = pred ? 16 : 0;  // src_size 0 -> ZFILL
                        asm volatile("cp.async.cg.shared.global [%0], [%1], 16, %2;\n"
                                     :: "r"(dst), "l"(gmem_src), "r"(src_size) : "memory");
                    };
                    auto copy_mean_chunk = [&](Element const* mean_base, auto block_stride,
                                               auto&& sXc, int const j0, int const n_log) {
                        static constexpr int kElemsPerCp = 16 / sizeof(Element);
                        int const groups_per_row = size<1>(sXc) / kElemsPerCp;
                        int const total = size<0>(sXc) * groups_per_row;
                        for (int i = thread_idx; i < total; i += NumProducerThreads) {
                            int const j = i / groups_per_row;
                            int const g = i % groups_per_row;
                            bool const pred = (j0 + j) < n_log;
                            cp_async_16B(&sXc(j, g * kElemsPerCp),
                                         mean_base + int64_t(j0 + j) * block_stride + g * kElemsPerCp,
                                         pred);
                        }
                    };
                    int csr_ptr = block_sparse_lo;
                    int const csr_end = block_sparse_lo + block_sparse_count;
                    int const n_chunks = (j_end + kBlockN - 1) / kBlockN;
                    for (int chunk = 0; chunk < n_chunks; ++chunk) {
                        int const j0 = chunk * kBlockN;
                        uint64_t sel_mask = 0;
                        while (csr_ptr < csr_end) {
                            int const t = params.ptr_block_sparse_idx[csr_ptr];
                            int const j = t / n_sub;
                            if (j >= j0 + kBlockN) { break; }
                            if (j >= j0) { sel_mask |= 1ull << (j - j0); }
                            csr_ptr += n_sub;
                        }
                        int const n_valid = std::min(kBlockN, j_end - j0);
                        uint64_t const valid = n_valid >= 64 ? ~0ull : ((1ull << n_valid) - 1ull);
                        uint64_t const todo = valid & ~sel_mask;
                        if (todo == 0ull) { continue; }
                        if (should_load_KV) {
                            pipeline_k.producer_acquire(smem_pipe_write);
                            copy_mean_chunk(k_mean_base, params.k_mean_block_stride,
                                            sK_pi(_, _, smem_pipe_write.index()), j0, n_logical);
                            pipeline_k.producer_commit(smem_pipe_write, cutlass::arch::cpasync_barrier_arrive);

                            auto pipeline_v_load = cute::conditional_return<!Transpose_V>(pipeline_v, pipeline_vt);
                            pipeline_v_load.producer_acquire(smem_pipe_write);
                            copy_mean_chunk(v_mean_base, params.v_mean_block_stride,
                                            sVcpasync(_, _, smem_pipe_write.index()), j0, n_logical);
                            pipeline_v_load.producer_commit(smem_pipe_write, cutlass::arch::cpasync_barrier_arrive);
                        }
                        if constexpr (Transpose_V) { copy_Vt_to_V(smem_pipe_write); }
                        ++smem_pipe_write;
    ...
    mma(Params const& params,
        ...
        if constexpr (Has_BlockSparse) {
            ...
            int g = params.total_q_tiles * bidh_kv + global_m_block;
            block_sparse_lo = params.ptr_block_sparse_cu[g];
            int block_sparse_hi = params.ptr_block_sparse_cu[g + 1];
            block_sparse_count = block_sparse_hi - block_sparse_lo;
            if (block_sparse_count == 0) { return false; }
            // Split-aware partition: divide the sparse K-tile list among splits
            if constexpr (Split) {
                uint32_t num_splits_dynamic_u = reinterpret_cast<uint32_t const&>(split_idx) >> 16;
                int num_splits_dynamic = reinterpret_cast<int&>(num_splits_dynamic_u);
                ...
                if (num_splits_actual > 1 && block_sparse_count > 0) {
                    ...
                    int split_start = split_idx_actual * blocks_per_split;
                    int split_end = std::min(split_start + blocks_per_split, block_sparse_count);
                    if (split_start >= split_end) { return false; }
                    block_sparse_lo += split_start;
                    block_sparse_count = split_end - split_start;
                }
            }
        ...
        if constexpr (IntraWGOverlap) {
            ...
            if constexpr (!Has_BlockSparse) { --n_block; }
            ...
            bool apply_scoremod = true;  // mean-correction chunk scores bypass the softcap scoremod
            auto fwd_step = [&](int const n_block, auto mask_fn, auto check_inf_type) {
                ...
                if (apply_scoremod) { scoremod_premask_fn(tSrS); }
            ...
            if constexpr (Has_BlockSparse) {
                if constexpr (Is_causal || Is_local) {
                    ...
                    int const n_block_min_before_local_mask = BlockMN_t::get_n_block_min_before_local_mask(
                        ...
                        params.attention_chunk_divmod, params.qhead_per_khead_divmod);
                    auto causal_local_mask_fn = [&](auto& tSrS, int n_block) {
                    ...
                    // Software pipeline: prefetch first index before the loop
                    int n_block_next = (block_sparse_count >= 2) ? params.ptr_block_sparse_idx[block_sparse_lo + block_sparse_count - 2] : 0;
                    #pragma unroll 1
                    for (int i = block_sparse_count - 2; i >= 0; --i) {
                        int n_block = n_block_next;
                        if (i > 0) { n_block_next = params.ptr_block_sparse_idx[block_sparse_lo + i - 1]; }
                        if (n_block >= n_block_min_causal_local_mask) {
                            fwd_step(n_block, causal_local_mask_fn, cute::true_type{} /*check_inf*/);
                        } else if (n_block >= n_block_min_before_local_mask) {
            ...
            // Zero-order mean correction for unselected blocks, folded into the
            // fwd_step pipeline: each kBlockN-wide chunk of pooled block means
            // is one extra iteration whose K/V was staged by the producer right
            // after the selected blocks. Chunks where every block is selected
            // are skipped with the same two-pointer scan over the ascending CSR
            // as the producer (a selected logical block j expands to n_sub
            // consecutive physical tiles starting at j * n_sub), so the pipeline
            // stays in lockstep. The score of each corrected block is shifted by
            // log2(block_len) so that one mean vector represents block_len
            // tokens in both the numerator (P_j * v_mean_j) and the denominator
            // (P_j = block_len * exp(score)). Causal safety: only blocks fully
            // visible to every row of this M tile are corrected, i.e.
            // (j+1)*kB <= prefix + q_pos_min; the diagonal band is left to the
            // exact path. Fwd only; disabled for Split, LargeHeadDimV, HasQv,
            // and AppendKV instantiations.
            if constexpr (Has_BlockSparse && PagedKVNonTMA && !LargeHeadDimV && !Split && !HasQv && !AppendKV) {
                if (params.ptr_k_mean != nullptr && params.mean_k_block_size > 0) {
                    int const kB = params.mean_k_block_size;
                    int const n_sub = params.mean_n_sub;
                    ...
                    int j_end = n_logical;
                    if constexpr (Is_causal) {
                        int const prefix = seqlen_k - seqlen_q;
                        int const q_pos_min = !PackGQA
                            ? m_block * kBlockM
                            : params.qhead_per_khead_divmod.divide(m_block * kBlockM);
                        // Must match the producer-side chunk staging exactly.
                        j_end = std::min(j_end, (prefix + q_pos_min) / kB);
                    }
                    if (j_end > 0) {
                        apply_scoremod = false;
                        float const inv_scale_log2 = 1.f / softmax.softmax_scale_log2;
                        float const full_len_shift = log2f(float(kB)) * inv_scale_log2;
                        int const tail_len = seqlen_k - (n_logical - 1) * kB;
                        float const tail_shift = log2f(float(tail_len)) * inv_scale_log2;
                        ...
                        for (int chunk = 0; chunk < n_chunks; ++chunk) {
                            ...
                            uint64_t const todo = valid & ~sel_mask;
                            if (todo == 0ull) { continue; }
                            auto corr_mask_fn = [&](auto& tSrS, int) {
                                Tensor tSrS_rowcol = make_tensor(tSrS.data(), flash::convert_layout_acc_rowcol(tSrS.layout()));
                                #pragma unroll
                                for (int i = 0; i < size(tSrS_rowcol); ++i) {
                                    int const j = j0 + get<1>(tScS_rowcol(i));
                                    if (((todo >> (j - j0)) & 1ull) != 0ull) {
                                        bool const is_tail = (j == n_logical - 1) && (tail_len != kB);
                                        tSrS_rowcol(i) += is_tail ? tail_shift : full_len_shift;
                                    } else {
                                        tSrS_rowcol(i) = -INFINITY;
                                    }
                                }
                            };
                            fwd_step(chunk, corr_mask_fn, cute::false_type{} /*check_inf*/);
                        }
                    }
                }
            }
            // All QK GEMMs (selected + correction) have completed here, so sQ
            // is no longer read; release it while the PV drain and epilogue run.
            cutlass::arch::NamedBarrier::arrive(NumMmaThreadsQK + (Use_TMA_Q ? cutlass::NumThreadsPerWarp : NumProducerThreads), static_cast<uint32_t>(FwdNamedBarriers::QueryEmpty) /*id*/);
            if constexpr (RescaleOBeforeGemm) { softmax.rescale_o(tOrO, scores_scale); }
            ...
            if constexpr (Has_BlockSparse) {
                if constexpr (Is_causal || Is_local) {
                    ...
                    for (int i = block_sparse_count - 2; i >= 0; --i) {
                        ...
                        if (i > 0) { n_block_next = params.ptr_block_sparse_idx[block_sparse_lo + i - 1]; }
                        if (n_block >= n_block_min_causal_local_mask) {
                            fwd_step(n_block, causal_local_mask_fn, cute::false_type{} /*is_first_iter*/, cute::true_type{} /*check_inf*/);
                        } else if (n_block >= n_block_min_before_local_mask) {
                    ...
                    #pragma unroll 1
                    for (int i = block_sparse_count - 2; i >= 0; --i) {
                        int n_block = params.ptr_block_sparse_idx[block_sparse_lo + i];
                        fwd_step(n_block, no_mask_fn, cute::false_type{} /*is_first_iter*/, cute::false_type{} /*check_inf*/);
                    }
                }
            } else {
            ...
            // Zero-order mean correction for unselected blocks, folded into the
            // fwd_step pipeline (see the IntraWGOverlap path for the detailed
            // comment). Consumes the mean chunks the producer appended after
            // the selected blocks; must run before finalize.
            if constexpr (Has_BlockSparse && PagedKVNonTMA && !LargeHeadDimV && !Split && !HasQv && !AppendKV) {
                if (params.ptr_k_mean != nullptr && params.mean_k_block_size > 0) {
                    int const kB = params.mean_k_block_size;
                    int const n_sub = params.mean_n_sub;
                    ...
                    if (j_end > 0) {
                        ...
                        for (int chunk = 0; chunk < n_chunks; ++chunk) {
                            ...
                            fwd_step(chunk, corr_mask_fn, cute::false_type{} /*is_first_iter*/, cute::false_type{} /*check_inf*/);
                        }
                    }
                }
            }
            // Advance past the last consumed K/V stage, matching the producer's
            // per-tile stage count (it always advances once after the selected
            // blocks, before staging the mean chunks).
            ++smem_pipe_read;
            warp_scheduler_barrier_arrive();
            // Tell producers that smem_q is ready
            cutlass::arch::NamedBarrier::arrive(NumMmaThreadsQK + (Use_TMA_Q ? cutlass::NumThreadsPerWarp : NumProducerThreads), static_cast<uint32_t>(FwdNamedBarriers::QueryEmpty) /*id*/);
